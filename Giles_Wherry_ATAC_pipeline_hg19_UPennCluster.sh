...
do
 sampleID="${i%.*}"
 bedtools coverage -counts -b $i -a ${sampleID}_peaks.bg > ${sampleID}_indCounts.bedgraph
done
...
do
 sampleID="${i%.*}"
 bedtools coverage -counts -b $i -a unionbed_merged.bed | bedtools sort -i - > ${sampleID}_counts.bedgraph
done
